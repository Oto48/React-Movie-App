...
const BookmarkLogo = ({ fill, width, height, viewBox, bookmarked }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className={`group hover:opacity-circle ${bookmarked ? "hover:fill-darkBlue" : "hover:fill-white"}`}
      width={width}
      height={height}
      viewBox={viewBox}
      fill={fill}
    >
      <circle
        className="opacity-50 group-hover:opacity-100 group-hover:fill-white"
        opacity="0.500647"
        cx="16"
        cy="16"
        r="16"
      ...
      <path
        ...
        className="group-hover:text-darkBlue stroke-current"
        strokeWidth="1.5"
      />
    </svg>
  );
